...
func SetupAxis(axis Axis, label string, flags AxisFlags) {
	if len(label) == 0 {
		...
		C.igpSetupAxis(C.igpAxis(axis), wrapStringBad(label), C.igpAxisFlags(flags))
	}
}
...
func SetupAxisFormat(axis Axis, fmt string) {
	C.igpSetupAxisFormat(C.igpAxis(axis), wrapStringBad(fmt))
}
...
func SetupAxes(xlabel, ylabel string, xflags, yflags AxisFlags) {
	C.igpSetupAxes(wrapStringBad(xlabel), wrapStringBad(ylabel), C.igpAxisFlags(xflags), C.igpAxisFlags(yflags))
}
